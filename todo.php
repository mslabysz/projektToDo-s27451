<?php
$host="localhost";
$dbname="logindb";
$username="root";
$password="";

$mysqli=new mysqli($host,$username,$password,$dbname);
if($mysqli->connect_errno){
    die("Błąd połączenia: ".$mysqli->connect_error);
}
return $mysqli;
$user_id=$_SESSION['user_id'];
$select="SELECT*FROM 'todolist' WHERE user_id='$user_id'";
$s_query=mysqli_query($conn,$select);
$tasks=$_POST['add-todo'];
$priority = $_POST['priority'];
//Dodawanie
if(isset($_POST['add-todo-btn'])){
    $insert = "INSERT INTO `todolist`(`user_id`, `tasks`, `created_at`, `updated_at`,'priority') VALUES ('$user_id','$tasks', CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'$priority')";
    $query = mysqli_query($conn,$insert);
    if($query){
        echo "<script> window.location='../../home.php';</script>";
    }else{
        echo "<script>alert('Coś poszło nie tak! Spróbuj ponownie.'); window.location='../../home.php';</script>";
    }
}
//Usuwanie
if(isset($_POST['dlt-todo-btn'])){
    $row_id = $_GET['id'];
    $delete = "DELETE FROM `todolist` WHERE id = $row_id";
    $d_query = mysqli_query($conn,$delete);
    if($d_query){
        echo "<script> window.location='../../home.php';</script>";
    }else{
        echo "<script>alert('Something Went wrong...try again!!'); window.location='../../home.php';</script>";
    }
}
?>